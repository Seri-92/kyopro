...
mini_r = [INF for i in range(H)]
mini_c = [INF for i in range(W)]

for i in range(N):
    a,b,c = mi()
    mini_r[a-1] = min(mini_r[a-1],c)
    mini_c[b-1] = min(mini_c[b-1],c)
    G.add_edge(a,b+H,1,c)

res = sum(mini_r) + sum(mini_c)

for i in range(H):
    G.add_edge(0,i+1,1,INF-mini_r[i])
for j in range(W):
    G.add_edge(j+H+1,H+W+1,1,INF-mini_c[j])
